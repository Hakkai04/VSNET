...
def get_args():
    parser = argparse.ArgumentParser(description="UNet Baseline Training Script")
    # 修改默认 project 和 name 以符合要求
    parser.add_argument("--project", default="runs/train", help="Save results to project/name")
    parser.add_argument("--name", default="unet", help="Save results to project/name")
    ...
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate (UNet通常可以用稍大一点)")
...
def get_transforms(patch_size, num_samples):
    # UNet 不需要加载 edge 和 reg，只保留 image 和 label
    keys = ["image", "label"]
...
def main():
    ...
    logger.info(f"🚀 UNet Baseline Training started. Results saved to {save_dir}")
    ...
    # 3. 模型构建 (标准 3D UNet)
    # 通道配置参考论文 Baseline 常用设置 (16, 32, 64, 128, 256)
    # 论文复现版 UNet 配置
    model = UNet(
        spatial_dims=3,
        in_channels=1,
        out_channels=3, # 背景, HV, PV
        channels=(32, 64, 128, 256, 512), # 通道数加倍
        strides=(2, 2, 2, 2),
        num_res_units=2,
        norm="batch", # 论文未明说，但 17.8M 符合带 BN 的标准 UNet
    ).to(device)
    
    params_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"UNet Model parameters: {params_num / 1e6:.2f}M")
